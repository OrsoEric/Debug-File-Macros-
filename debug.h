	...
	**********************************************************************************/
		...
		#define DSTOP()	\
			((_debug_file != NULL)?fprintf(_debug_file, "\nDebug has Ended!\n"), fclose(_debug_file), _debug_file = NULL:(0))
		...
		#define DRETURN()	\
			(_debug_indent_level>0)?(--_debug_indent_level):(0), DPRINT( "Return >%s<\n", __FUNCTION__)

		//Enter Function and increase indent level. No argument print version
		#define DENTER_ARG( ... )	\
			DPRINT( "Enter >%s< | ", __FUNCTION__), DPRINT_NOTAB( __VA_ARGS__ ), (_debug_indent_level<_DEBUG_MAX_INDENT_LEVEL)?(++_debug_indent_level):(_DEBUG_MAX_INDENT_LEVEL)

		//Return from function and decrease indent level. No argument version
		#define DRETURN_ARG( ... )	\
			(_debug_indent_level>0)?(--_debug_indent_level):(0), DPRINT( "Return >%s< | ", __FUNCTION__), DPRINT_NOTAB( __VA_ARGS__ )
